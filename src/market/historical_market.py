...
class HistoricalMarket:
    ...
    def get_quotes(self) -> list[MarketQuote]:
        ...
        for i, venue in enumerate(self._venues):
            ...
            half_spread = mid_price * (fee_bps / BPS_DIVISOR / TWO)
    ...
    def _extract_price(self, snapshot: dict, venue: dict) -> Decimal:
        ...
        if close and D(str(close)) > ZERO:
            return D(str(close))

        # Fall back to raw token price fields from the subgraph.
        t0p = D(str(snapshot.get("token0Price", 0)))
        t1p = D(str(snapshot.get("token1Price", 0)))
    ...
    def _find_fee_bps(self, dex_name: str, chain: str) -> Decimal:
        ...
        for dex in self.config.dexes:
            if dex.dex_type == dex_name or dex.name.lower().startswith(dex_name.split("_")[0]):
                return dex.fee_bps
        return D("30")  # default Uniswap 0.30% tier
