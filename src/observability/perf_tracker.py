...
Usage::
    ...
    PYTHONPATH=src python -m perf_tracker --file logs/bot_2026-04-13_02-57-00.jsonl
...
class PerfReport:
    ...
    # PnL metrics (Decimal for financial precision)
    total_expected_profit: Decimal = ZERO
    total_realized_profit: Decimal = ZERO
    max_single_profit: Decimal = ZERO
...
def analyze_jsonl(filepath: str | Path) -> PerfReport:
    ...
    for record in iter_json_records(path):
        ...
        if event == "scan":
            ...
            if opp:
                expected = D(str(opp.get("net_profit_base", 0)))
            ...
            if record.get("success"):
                ...
                report.total_realized_profit += D(str(record.get("realized_profit_base", 0)))
            else:
