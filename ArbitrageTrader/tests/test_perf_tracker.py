...
class AnalyzeJsonlTests(unittest.TestCase):
    ...
    def test_tracks_execution_success_and_failure(self) -> None:
        ...
        self.assertAlmostEqual(float(report.total_realized_profit), 0.02)
    ...
    def test_tracks_expected_profit(self) -> None:
        ...
        report = analyze_jsonl(path)
        self.assertAlmostEqual(float(report.total_expected_profit), 0.08)
        self.assertAlmostEqual(float(report.total_realized_profit), 0.065)
        self.assertAlmostEqual(float(report.max_single_profit), 0.05)
