...
Usage::
    ...
    # Search by symbol
    PYTHONPATH=src python -m pair_scanner --token WETH --chain ethereum

    # Search by contract address (more precise)
    PYTHONPATH=src python -m pair_scanner \\
...
def discover_pairs_for_bot(
    ...
    default_sizes: dict[str, _D] = {
        "WBTC/USDC": _D("0.05"), "WBTC/USDT": _D("0.05"),
    }
    ...
    for pair_name, pairs in sorted_pairs:
        ...
        trade_size = sizes.get(pair_name, _D("1"))
