package com.hulu.harmony.core.config;
...
public class ConfigBuilder {
  ...
  private final Optional<String> originDesciption;
  private Config currentConfig;

  ConfigBuilder(Optional<String> originDescription) {
    this.originDesciption = originDescription;
    this.currentConfig =
        originDescription.isPresent() ? ConfigFactory.empty(this.originDesciption.get()) : ConfigFactory.empty();
  }
  ...
  public ConfigBuilder loadProps(Properties props, String scopePrefix) {
    ...
    for (Map.Entry<Object, Object> propEntry : props.entrySet()) {
      ...
      if (propName.startsWith(scopePrefix)) {
        ...
        if (!Character.isLetter(scopedName.charAt(0))) {
          throw new RuntimeException(
              "Scoped name for property " + propName + " should start with a character: " + scopedName);
        }
        this.properties.put(scopedName, propEntry.getValue());
      }
    }
  ...
  public ConfigBuilder addList(String name, Iterable<? extends Object> values) {
    this.currentConfig = this.originDesciption.isPresent()
        ? this.currentConfig.withValue(name, ConfigValueFactory.fromIterable(values, this.originDesciption.get()))
  ...
  public Config build() {
    return ConfigFactory.parseMap(this.properties).withFallback(this.currentConfig);
  }
