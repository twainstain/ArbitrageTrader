...
Usage::
    ...
    PYTHONPATH=src python -m log_parser

    # Parse a specific log file
    PYTHONPATH=src python -m log_parser logs/bot_2026-04-13_00-10-53.jsonl

    # Show only opportunities (skip empty scans)
    PYTHONPATH=src python -m log_parser --opportunities-only

    # Show full quote details
    PYTHONPATH=src python -m log_parser --show-quotes

    # Export parsed output to a file
    PYTHONPATH=src python -m log_parser --output data/parsed_report.txt
"""
...
def _n(val: object) -> float:
    """Coerce a value (possibly a Decimal-encoded string) to float for display."""
...
def format_scan(record: dict, show_quotes: bool = False) -> str:
    ...
    if show_quotes and quotes:
        ...
        for pair, pqs in sorted(by_pair.items()):
            ...
            for q in pqs:
                ...
                sp = _n(q["sell_price"])
                mid = (bp + sp) / 2
                vol = _n(q.get("volume_usd", 0))
                liq = _n(q.get("liquidity_usd", 0))
                extra = ""
                if vol > 0:
                    extra += f"  vol=${vol:,.0f}"
                if liq > 0:
                ...
                lines.append(
                    ...
                    f"buy=${bp:>12,.4f}  sell=${sp:>12,.4f}{extra}"
                )

            # Compute best spread within this pair.
            if len(pqs) >= 2:
                cheapest = min(pqs, key=lambda x: _n(x["buy_price"]))
                priciest = max(pqs, key=lambda x: _n(x["sell_price"]))
                if cheapest["dex"] != priciest["dex"]:
                    spread = _n(priciest["sell_price"]) - _n(cheapest["buy_price"])
                    pct = spread / _n(cheapest["buy_price"]) * 100 if _n(cheapest["buy_price"]) else 0
                    lines.append(
    ...
    if opp:
        ...
        lines.append(
            f"    spread={_n(opp.get('gross_spread_pct', 0)):.4f}%  "
            f"net_profit={_n(opp['net_profit_base']):.6f}"
        )
        lines.append(
            f"    cost_to_buy=${_n(opp.get('cost_to_buy_quote', 0)):,.4f}  "
            f"proceeds=${_n(opp.get('proceeds_from_sell_quote', 0)):,.4f}"
        )
        lines.append(
            f"    dex_fees=${_n(opp.get('dex_fee_cost_quote', 0)):,.4f}  "
            f"flash_fee=${_n(opp.get('flash_loan_fee_quote', 0)):,.4f}  "
            f"slippage=${_n(opp.get('slippage_cost_quote', 0)):,.4f}  "
            f"gas={_n(opp.get('gas_cost_base', 0))}"
        )
...
def format_execution(record: dict) -> str:
    ...
    profit = _n(record.get("realized_profit_base", 0))
...
def format_summary(record: dict) -> str:
    ...
    lines.append(f"  Total realized profit:{_n(record.get('total_realized_profit', 0)):>10.6f} {record.get('base_asset', '')}")
    return "\n".join(lines)
